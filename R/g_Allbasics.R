...
#' @param frames Column name in list_subjects representing the frame number or time stamp.
...
g_Allbasics <- function(list_subjects, list_dadoz = NULL, Xaxi = "x_cm", Yaxi = "y_cm", Zaxi = NA, frames = "frame", fps = 30,
  ...
  if(base::is.null(list_dadoz)){
    list_subjects <- base::lapply(list_subjects, FUN = Allbasics, dadoz = list_dadoz, Xaxi = Xaxi, Yaxi = Yaxi, Zaxi = Zaxi, frames = frames,
    ...
    if(base::length(list_subjects) == base::length(list_dadoz)){
      for (i in 1:base::length(list_dadoz)) {
        ...
        list_subjectsz <- Allbasics(dado = list_subjects[[i]], dadoz = list_dadoz[[i]], Xaxi = Xaxi, Yaxi = Yaxi, Zaxi = Zaxi, frames = frames,
                                    fps = fps, id_col = id_col, id = id, Zones = Zones, n.zonesd2 = n.zonesd2, n.zonesd3 = n.zonesd3,
                                    faceZ = faceZ, maxX = maxX, minX = minX, maxY = maxY, minY = minY, maxZ = maxZ, minZ = minZ,
                                    npts = npts, Circ_Arena = Circ_Arena, threshold = threshold, Dist.reg = Dist.reg, reg = reg)
